...
export default class Keycloak {
  ...
  init = async (initOptions = {}) => {
    ...
    const adapters = ['default', 'cordova', 'cordova-native', 'capacitor']
    ...
    if (typeof initOptions.adapter === 'string' && adapters.includes(initOptions.adapter)) {
      ...
      this.#adapter = this.#loadAdapter('capacitor')
  ...
  /**
   * @param {"default" | "cordova" | "cordova-native" | "capacitor"} type
  ...
  #loadAdapter (type) {
    ...
    if (type === 'capacitor') {
      this.#loginIframe.enable = false
      return this.#loadCapacitorAdapter()
    }
  ...
  #loadCapacitorAdapter () {
     ...
     * @returns {Promise<any>}
     */
    const capacitorOpenWindowWrapper = async (loginUrl, target, options) => {
      if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.InAppBrowser) {
        // Use @capgo/inappbrowser plugin for Capacitor
        return await window.Capacitor.Plugins.InAppBrowser.open({
          url: loginUrl,
          target,
          options
        })
    ...
    const shallowCloneCapacitorOptions = (userOptions) => {
      if (userOptions && userOptions.cordovaOptions) {
        return Object.keys(userOptions.cordovaOptions).reduce((options, optionName) => {
          options[optionName] = userOptions.cordovaOptions[optionName]
          return options
    ...
    const formatCapacitorOptions = (capacitorOptions) => {
      return Object.keys(capacitorOptions).reduce((options, optionName) => {
        options.push(optionName + '=' + capacitorOptions[optionName])
        return options
      }, []).join(',')
    }

    const createCapacitorOptions = (userOptions) => {
      const capacitorOptions = shallowCloneCapacitorOptions(userOptions)
      capacitorOptions.location = 'no'
      if (userOptions && userOptions.prompt === 'none') {
        capacitorOptions.hidden = 'yes'
      }
      return formatCapacitorOptions(capacitorOptions)
    }

    const getCapacitorRedirectUri = () => {
      return this.redirectUri || 'http://localhost'
    }

    return {
      login: async (options) => {
        const capacitorOptions = createCapacitorOptions(options)
        const loginUrl = await this.createLoginUrl(options)
        const ref = await capacitorOpenWindowWrapper(loginUrl, '_blank', capacitorOptions)
        let completed = false
        let closed = false
        ...
        return await new Promise((resolve, reject) => {
          const handleLoadStart = async (event) => {
            if (event.url.indexOf(getCapacitorRedirectUri()) === 0) {
              const callback = this.#parseCallback(event.url)
          ...
          const handleLoadError = async (event) => {
            if (!completed) {
              if (event.url.indexOf(getCapacitorRedirectUri()) === 0) {
                ...
                closeBrowser()
                reject(new Error('InAppBrowser loadError'))
              }
            }
          }
          ...
          if (ref && ref.addEventListener) {
            ...
            ref.addEventListener('loaderror', handleLoadError)
      ...
      logout: async (options) => {
        const logoutUrl = this.createLogoutUrl(options)
        const ref = await capacitorOpenWindowWrapper(logoutUrl, '_blank', 'location=no,hidden=yes,clearcache=yes')
        let error = false
        ...
        await new Promise((resolve, reject) => {
          const handleExit = () => {
            if (error) {
              reject(new Error('An error occurred while logging out'))
            } else {
              this.clearToken()
              resolve()
            }
      ...
      register: async (options) => {
        const registerUrl = await this.createRegisterUrl()
        const capacitorOptions = createCapacitorOptions(options)
        const ref = await capacitorOpenWindowWrapper(registerUrl, '_blank', capacitorOptions)
      ...
      accountManagement: async () => {
        const accountUrl = this.createAccountUrl()
        if (typeof accountUrl !== 'undefined') {
          const ref = await capacitorOpenWindowWrapper(accountUrl, '_blank', 'location=no')
          const handleLoadStart = function (event) {
            if (event.url.indexOf(getCapacitorRedirectUri()) === 0) {
              if (ref && ref.close) {
                ref.close()
              }
      ...
      redirectUri: () => {
        return getCapacitorRedirectUri()
      }
    }
  }
