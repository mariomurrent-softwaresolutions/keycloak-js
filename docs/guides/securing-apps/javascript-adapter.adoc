...
[#hybrid-apps-with-capacitor]
== Hybrid Apps with Capacitor

{project_name} supports hybrid mobile apps developed with https://capacitorjs.com/[Capacitor]. The adapter provides built-in support for Capacitor using the `capacitor` adapter mode.

The `capacitor` adapter mode works similarly to the Cordova adapter but is specifically designed for Capacitor apps. It automatically detects when running in a Capacitor environment (when `window.Capacitor` is present) and uses the https://www.npmjs.com/package/@capgo/inappbrowser[@capgo/inappbrowser] plugin to open an InApp Browser for authentication.

To use this adapter, first install the required Capacitor plugin:
...
The adapter will automatically select the `capacitor` mode when `window.Capacitor` is detected. Alternatively, you can explicitly specify it:
...
When logging in, the adapter opens an InApp Browser that lets the user interact with {project_name} and afterwards returns to the app by redirecting to `http://localhost`. Make sure to whitelist this URL as a valid redirect URI in the client configuration section of the Admin Console.

Similar to the Cordova adapter, there are some considerations:

* The InApp Browser is embedded in the app and is not the phone's default browser, so stored credentials from the system browser will not be available.
* The InApp Browser might be slower when rendering complex themes.
* Security considerations apply as the app has control over the browser rendering the login page.

[NOTE]
====
If you need to use the system browser instead of an InApp Browser for better security and user experience, you may want to implement a custom adapter using Capacitor's Browser API along with App Links (Android) or Universal Links (iOS) for deep linking back to your app.
====

[#custom-adapters]
== Custom Adapters

In some situations, you may need to run the adapter in environments that are not supported by default. To use the JavaScript client in these environments, you can pass a custom adapter. For example, a third-party library could provide such an adapter to make it possible to reliably run the adapter in a specific environment.

It's possible to make your own adapter, to do so you will have to implement the methods described in the `KeycloakAdapter` interface. For example the following TypeScript code ensures that all the methods are properly implemented:
...
adapter::
    ...
    * "capacitor" - the library will use the @capgo/inappbrowser plugin for Capacitor apps (this is used automatically when the library detects a Capacitor environment)
...
* cordovaOptions - Specifies the arguments that are passed to the Cordova or Capacitor in-app-browser (if applicable). Options `hidden` and `location` are not affected by these arguments. All available options are defined at https://cordova.apache.org/docs/en/latest/reference/cordova-plugin-inappbrowser/. Example of use: `{ zoom: "no", hardwareback: "yes" }`;
...
* *onAuthLogout* - Called if the user is logged out (will only be called if the session status iframe is enabled, or in Cordova/Capacitor mode).
